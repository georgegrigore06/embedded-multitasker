...
/*!
 * @brief OLED data transmitter.
 * @param buffer Information to be sent
 * @param size Buffer size
 * @param type OLED can receive information in the form of commands or data
 */
void sendOLED(uint8_t *buffer, uint16_t size, uint8_t type)
{
	masterxfer.subaddress = (uint32_t) type;
	masterxfer.data = buffer;
	masterxfer.dataSize = size;
	LPI2C_MasterTransferEDMA(I2C_OLED, &LP_FLEXCOMM2_master_handle, &masterxfer);
...
void initOLED()
{
	...
	masterxfer.slaveAddress   = OLED_ADDRESS;
	...
	for(uint32_t strChar = 0; buffer[strChar] != '\0'; strChar++)
	{
		...
		switch(buffer[strChar])
		{
			case '\n':
				...
				break;
			default:
				writeChar(buffer[strChar]); // write character to display
...
-------------------------------------------------------------------------------------------------*/
void writeString(char *string, uint8_t seg, uint8_t page)
{
	setPage(page);
	setSeg(seg);
...
-------------------------------------------------------------------------------------------------*/
void printVar(char* formatting, int32_t var, uint8_t seg, uint8_t page)
{
	char pageBuffer[21];
	sprintf(pageBuffer, formatting, var);
	writeString(pageBuffer, seg, page);
} /* printVar() */
